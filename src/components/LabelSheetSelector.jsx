...
import { LABEL_SHEETS } from '../config/labelSheets';

export default function LabelSheetSelector({ selectedSheetId, onSelectSheet, disabled = false }) {
  return (
    <div className={`space-y-3 transition-opacity duration-300 ${disabled ? 'opacity-60' : 'opacity-100'}`}>
      ...
      <div className={`flex items-center space-x-4 p-4 bg-white border border-gray-200 rounded-lg shadow-sm ${disabled ? 'pointer-events-none opacity-50' : ''}`}>
        <div className="w-12 h-12 rounded-lg border border-gray-300 shadow-inner flex-shrink-0 bg-gradient-to-br from-gray-50 to-gray-200" />

        <div className="flex-1 space-y-1">
          <label htmlFor="label-sheet-dropdown" className="text-xs font-semibold text-gray-400 block">
            Paper stock:
          </label>
          <div className="relative">
            <select
              id="label-sheet-dropdown"
